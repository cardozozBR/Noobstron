...
@section('title', __('ui.users.edit_title'))
...
<div class="user-form-page">
    <div class="page-header">
        <div>
            ...
            <h1>{{ __('ui.users.edit_heading') }}</h1>

            <p>
                {{ __('ui.users.tenant_label') }}
    ...
    <div class="card">
        <form method="POST" action="{{ route('users.update', $user->id) }}">
            ...
            <div class="section-header">
                <div>
                    <span class="eyebrow">{{ __('ui.users.security') }}</span>
                    <h2>{{ __('ui.users.change_password') }}</h2>
                    <p>{{ __('ui.users.password_help') }}</p>
                </div>
            </div>

            <div class="form-group">
                <label for="password" class="form-label">
                    {{ __('ui.users.new_password') }}
                </label>

                <input id="password" type="password" name="password" class="form-control">

                <span class="form-help">
                    {{ __('ui.users.password_help') }}
                </span>
            </div>

            <div class="form-group">
                <label for="password_confirmation" class="form-label">
                    {{ __('ui.users.confirm_new_password') }}
                </label>
